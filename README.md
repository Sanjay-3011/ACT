...
Collecting expert demonstrations of pick-and-place using a custom IK-based controller.
Training an ACT policy on those demonstrations to imitate the behavior visually (from camera images), not from privileged simulator state.
Evaluating the trained policy in closed-loop rollout to see if it can actually complete the task on its own.
How I'm Doing It
